...
UXS_EXPORT bool operator==(const basic_value<CharT, Alloc>& lhs, const basic_value<CharT, Alloc>& rhs) noexcept {
    ...
    switch (lhs.type_) {
        ...
        case dtype::string: return rhs.type_ == dtype::string && lhs.value_.str.view() == rhs.value_.str.view();
        case dtype::array: {
            if (rhs.type_ != dtype::array) { return false; }
            const auto lha = lhs.value_.arr.view();
            const auto rha = rhs.value_.arr.view();
            return lha.size() == rha.size() && std::equal(lha.begin(), lha.end(), rha.begin());
        } break;
        case dtype::record: {
...
/*static*/ auto flexarray_t<Ty, Alloc>::alloc(alloc_type& al, std::size_t cap) -> data_t* {
...
void flexarray_t<Ty, Alloc>::grow(alloc_type& al, std::size_t extra) {
    ...
    data_t* p_new = alloc(al, p_->size + delta_sz);
...
template<typename Ty, typename Alloc>
void flexarray_t<Ty, Alloc>::rotate_back(std::size_t pos) {
...
void flexarray_t<Ty, Alloc>::resize(alloc_type& al, std::size_t sz, const Ty& v) {
    ...
    if (sz <= p_->size) {
        ...
        std::for_each(p_->data() + p_->size, p_->data() + sz, [&v](Ty& item) { new (&item) Ty(v); });
    }
    p_->size = sz;
}

template<typename Ty, typename Alloc>
void flexarray_t<Ty, Alloc>::erase(Ty* item) {
    assert(p_ && item >= p_->data() && item < p_->data() + p_->size);
    Ty* last = p_->data() + --p_->size;
    for (; item != last; ++item) { *item = std::move(*(item + 1)); }
    item->~Ty();
}
...
void basic_value<CharT, Alloc>::insert(std::size_t pos, std::initializer_list<basic_value> init) {
    ...
    typename value_array_t::alloc_type arr_al(*this);
    value_.arr.insert(arr_al, pos, init.begin(), init.end());
}
...
est::optional<bool> basic_value<CharT, Alloc>::get_bool() const {
    switch (type_) {
        ...
        case dtype::string: {
            ...
            return from_basic_string(value_.str.view(), *result) ? result : est::nullopt();
        } break;
...
est::optional<std::basic_string<CharT>> basic_value<CharT, Alloc>::get_string() const {
    switch (type_) {
        ...
        case dtype::string: return est::make_optional<std::basic_string<CharT>>(value_.str.view());
        case dtype::array: return est::nullopt();
        case dtype::record: return est::nullopt();
        default: UXS_UNREACHABLE_CODE;
    }
...
void basic_value<CharT, Alloc>::resize(std::size_t sz) {
    ...
    typename value_array_t::alloc_type arr_al(*this);
    value_.arr.resize(arr_al, sz, basic_value(get_allocator()));
}
...
void basic_value<CharT, Alloc>::erase(std::size_t pos) {
    ...
    value_.arr.erase(&value_.arr[pos]);
}
...
auto basic_value<CharT, Alloc>::erase(const_iterator it) -> iterator {
    ...
    uxs_iterator_assert(it.begin_ == value_.arr.debug_begin() && it.end_ == value_.arr.debug_end());
    value_.arr.erase(item);
    return iterator(item, value_.arr.debug_begin(), value_.arr.debug_end());
}
...
void basic_value<CharT, Alloc>::init_from(const basic_value& other) {
    switch (other.type_) {
        case dtype::string: {
            const auto s = other.value_.str.view();
            typename char_array_t::alloc_type str_al(*this);
            value_.str.init(str_al, s.begin(), s.end());
        } break;
        case dtype::array: {
            const auto arr = other.value_.arr.view();
            typename value_array_t::alloc_type arr_al(*this);
            value_.arr.init(arr_al, arr.begin(), arr.end());
        } break;
        case dtype::record: {
            typename record_t::alloc_type rec_al(*this);
...
void basic_value<CharT, Alloc>::assign_from(const basic_value& other) {
    ...
    switch (other.type_) {
        ...
        case dtype::array: {
            const auto arr = other.value_.arr.view();
            typename value_array_t::alloc_type arr_al(*this);
            value_.arr.assign(arr_al, arr.begin(), arr.end());
        } break;
        case dtype::record: {
            typename record_t::alloc_type rec_al(*this);
...
void basic_value<CharT, Alloc>::destroy() {
    switch (type_) {
        case dtype::string: {
            typename char_array_t::alloc_type str_al(*this);
            value_.str.destroy(str_al);
        } break;
        case dtype::array: {
            typename value_array_t::alloc_type arr_al(*this);
            value_.arr.destroy(arr_al);
        } break;
        case dtype::record: {
            typename record_t::alloc_type rec_al(*this);
