...
int basic_devbuf<CharT, Alloc>::flush_compressed_buf() {
    ...
    std::size_t sz = 0;
    dev_->advance(static_cast<std::size_t>(buf_->zstr.next_out - buf_->z_buf));
    ...
    do {
        if (!(buf_->flags & detail::devbuf_impl_flags::z_in_finish) && !buf_->zstr.avail_in) {
            if (!(dev_->caps() & iodevcaps::mappable)) {
                ...
                dev_->advance(static_cast<std::size_t>(buf_->zstr.next_in - buf_->z_buf));
                std::size_t sz = 0;
...
typename basic_devbuf<CharT, Alloc>::pos_type basic_devbuf<CharT, Alloc>::seek_impl(off_type off, seekdir dir) {
    ...
    if (dir == seekdir::curr) {
        const off_type delta = static_cast<off_type>(!!(this->mode() & iomode::out) ? this->pos() : -this->avail());
        if (off == 0) {
