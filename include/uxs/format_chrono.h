...
void format_chrono_day(FmtCtx& ctx, std::chrono::day d, const chrono_specs& specs) {
    if (!specs.modifier) {
        switch (specs.spec) {
            case chrono_specifier::day_zero: return format_chrono_day_dd(ctx, d, '0');
            case chrono_specifier::day_space: return format_chrono_day_dd(ctx, d, ' ');
            default: break;
        }
    }
...
void format_chrono_day_of_the_year(FmtCtx& ctx, std::chrono::year_month_day ymd) {
    ...
    scvt::fmt_integer(ctx.out(), yday, fmt_opts{fmt_flags::leading_zeroes, -1, 3});
}
...
void format_chrono_hours_12(FmtCtx& ctx, std::chrono::hours h) {
    const int hours = std::abs(h.count() % 24);
    const int hours_12 = hours > 0 ? (hours <= 12 ? hours : hours - 12) : 12;
    scvt::fmt_integer(ctx.out(), hours_12, fmt_opts{fmt_flags::leading_zeroes, -1, 2});
}
...
void format_chrono_seconds(
    FmtCtx& ctx, std::chrono::hh_mm_ss<Duration> hms,
    const std::enable_if_t<std::chrono::hh_mm_ss<Duration>::fractional_width == 0, fmt_opts>& opts) {
    const int seconds = static_cast<int>(hms.seconds().count());
...
struct formatter<std::chrono::day, CharT>
 ...
 private:
    ...
    static void default_value_writer(FmtCtx& ctx, value_type d, const fmt_opts&) {
        detail::format_chrono_day_dd(ctx, d, '0');
        if (d.ok()) { return; }
        ctx.out() += string_literal<typename FmtCtx::char_type, ' ', 'i', 's', ' ', 'n', 'o', 't', ' ', 'a', ' ', 'v',
                                    'a', 'l', 'i', 'd', ' ', 'd', 'a', 'y'>{}();
    }
 ...
 private:
    ...
    static void value_writer(FmtCtx& ctx, value_type t, const detail::chrono_specs& specs) {
        if (specs.spec == detail::chrono_specifier::time_zone) {
            ...
            const int hours = offset.hours().count();
            const int minutes = offset.minutes().count();
            scvt::fmt_integer(ctx.out(), offset.is_negative() ? -hours : hours,
                              fmt_opts{fmt_flags::leading_zeroes | fmt_flags::sign_pos, -1, 3});
            ctx.out() += '0' + minutes / 10;
            ctx.out() += '0' + minutes % 10;
        } else if (specs.spec == detail::chrono_specifier::time_zone_abbreviation) {
            ctx.out() += t.abbrev;
        } else {
