...
struct guid {
    ...
    struct data64_t {
        ...
        explicit operator bool() const { return q[0] || q[1]; }
        std::uint64_t operator[](std::size_t n) const { return q[n]; }
        friend bool operator==(data64_t lhs, data64_t rhs) noexcept {
            return lhs.q[0] == rhs.q[0] && lhs.q[1] == rhs.q[1];
        }
        friend bool operator<(data64_t lhs, data64_t rhs) noexcept {
            return lhs.q[0] < rhs.q[0] || (lhs.q[0] == rhs.q[0] && lhs.q[1] < rhs.q[1]);
        }
    };

    struct layout_t {
        UXS_CONSTEXPR layout_t() noexcept = default;
    ...
    bool valid() const noexcept { return !!data64(); }
