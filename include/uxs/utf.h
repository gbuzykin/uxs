...
template<typename OutputIt>
unsigned to_utf8_n(std::uint32_t code, OutputIt out, std::size_t n) {
    if (n == 0) { return 0; }
    if (code < 0x80) {
        *out++ = static_cast<std::uint8_t>(code);
        return 1;
...
template<typename OutputIt>
unsigned to_utf16(std::uint32_t code, OutputIt out) {
    if (code >= 0x10000) {
        if (code < 0x110000) {
            code -= 0x10000;
...
template<typename OutputIt>
unsigned to_utf16_n(std::uint32_t code, OutputIt out, std::size_t n) {
    if (n == 0) { return 0; }
    if (code >= 0x10000) {
        if (code < 0x110000) {
            if (n <= 1) { return 0; }
...
inline unsigned get_utf8_byte_count(std::uint8_t ch) {
    if (ch >= 0xc0 && ch < 0xf8) { return ch < 0xe0 ? 2 : (ch < 0xf0 ? 3 : 4); }
    return 1;
}

inline unsigned get_utf16_word_count(std::uint16_t ch) { return (ch & 0xdc00) == 0xd800 ? 2 : 1; }
